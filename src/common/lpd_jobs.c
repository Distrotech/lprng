...
"$Id: lpd_jobs.c,v 1.25 2001/10/15 13:25:30 papowell Exp $";
	...
	case JFAIL:	/* failed, retry ?*/
		...
		if( status == JFAIL ){
			if( Send_try_DYN > 0 ){
				...
				setstatus( job, _("job '%s' attempt %d, trying indefinately"),
					id, attempt);
			}
			if( destination ){
