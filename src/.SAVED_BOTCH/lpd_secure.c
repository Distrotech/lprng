...
int Receive_secure( int *sock, char *input )
{
	...
	if( Setup_printer( printername, error+1, sizeof(error)-1, 0 ) ){
		...
		if( !(DRECVMASK & DbgFlag) ){
			...
			DbgFlag = dbf;
			if( Log_file_DYN ){
				int tempfd = Checkwrite( Log_file_DYN, &statb,0,0,0);
	...
	DEBUGFC(DRECV1)Dump_line_list("Receive_secure - header", &header_info );
	...
	if( security->server_accept ){
		DEBUGF(DRECV1)("Receive_secure: using server_accept");
		status = security->server_accept( sock,
			user, jobsize, from_server, authtype,
			error+1, sizeof(error)-1,
			&info, &header_info, security );
		if( status ) goto error;
		if( security->flags & SEC_AUTH_EXCHANGE ){
			char *cmd = Find_str_value( &header_info, INPUT, Value_sep);
			if( !ISNULL(cmd) ){
				safestrncpy(error, cmd);
			} else {
				/* assume that we have a block transfer */
				SNPRINTF(error,sizeof(error)) "%c%s",
					REQ_BLOCK, Printer_DYN );
			}
			Free_line_list( &args );
			Free_line_list( &header_info );
			Free_line_list( &info );
			Dispatch_input( sock, error );
			cleanup(0);
		}
	}
	if( security->server_receive ){
		DEBUGF(DRECV1)("Receive_secure: using server_receive");
	...
	if( security->server_receive_done ){
		DEBUGF(DRECV1)("Receive_secure: using server_receive_done");
		status = security->server_receive_done( sock,
			error+1, sizeof(error)-1,
			&info, &header_info, security );
		if( status ) goto error;
	}
