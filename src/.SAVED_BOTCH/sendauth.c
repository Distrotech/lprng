	...
	struct security *security, struct line_list *info )
{
	...
	/* make up the string to send as the information */

	if( safestrcmp(security->config_tag, "kerberos") ){
		cmdval = Put_in_auth(cmdval,DESTINATION,destination);
		if( Is_server ) cmdval = Put_in_auth(cmdval,SERVER,from);
		cmdval = Put_in_auth(cmdval,CLIENT,client);
		if( cmd ){
			cmdval = Put_in_auth(cmdval,INPUT,cmd);
		} else if( job ){
			char *filename = Find_str_value( info, PRINTFILE, Value_sep );
			double size;
			if( ISNULL(filename) ){
				Errorcode = JABORT;
				FATAL(LOG_INFO) "Send_auth_transfer: missing PRINTFILE");
			}
			if( stat( filename, &statb ) ){
				Errorcode = JABORT;
				LOGERR_DIE(LOG_INFO) "Send_auth_transfer: stat PRINTFILE '%s' failed");
			}
			size = statb.st_size;
			SNPRINTF(buffer,sizeof(buffer)) "%c%s %0.0f",
				REQ_BLOCK, RemotePrinter_DYN, size );
			cmdval = Put_in_auth(cmdval,INPUT,buffer);
		}
	} else {
		if( cmd ){
			cmdval = Put_in_auth(cmdval,0,cmd);
		}
		if( Is_server ){
			cmdval = Put_in_auth(cmdval,0,client);
		}
	}
	Set_str_value(info,CMDVAL, cmdval );
	secure = Find_str_value(info,CMD, Value_sep);
	/* send the message */
     ...
     */
	if( security->client_connect ){
		status = security->client_connect( job, sock, error, errlen,
			info, security );
		if( status || security->flags & SEC_AUTH_EXCHANGE ){
			/* we now simply return */
			goto error;
		}
	}
	if( security->client_send ){
		status = security->client_send( job, sock, transfer_timeout,
			error, errlen, security, info );
		if( status ) goto error;
	}
	if( security->client_send_done ){
		status = security->client_send_done( job, sock, transfer_timeout,
			error, errlen, security, info );
		if( status ) goto error;
	}
 ...
 error:
	...
	if( cmdval ) free(cmdval); cmdval = 0;
	Free_line_list(info);
	...
	struct job *job, char *error, int errlen )
{
	...
	if( job ){
		double size = Find_double_value(&job->info, SIZE, Value_sep );
		n = strlen(buffer);
		SNPRINTF( buffer+n, sizeof(buffer)-n) " %0.0f", size );
	}
...
char *Put_in_auth( char *str, const char *key, char *value )
{
	char *v = Escape(value,1);
	DEBUG1("Put_in_auth: key '%s' value '%s', v '%s'",
		key, value, v );
	if( !ISNULL(key) ){
		str = safeextend3(str,key,"=", __FILE__,__LINE__ );
	}
	str = safeextend3(str,v,"\n", __FILE__,__LINE__ );
	if( v ) free(v); v = 0;
	return( str );
}
