	...
	int transfer_timeout, char *final_filter )
{
	...
	Set_str_value(&info,PRINTFILE,tempfile);
	if( job && security && !(security->flags & SEC_AUTH_SOCKET) ){
 ...
 retry_connect:
	...
	if( security ){
		if( security->flags & SEC_AUTH_SOCKET ){
			status = security->client_connect( job, &sock,
				error, sizeof(error), &info, security );
			if( status ) goto error;
		} else {
